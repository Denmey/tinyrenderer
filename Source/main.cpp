...
void line(int x0, int y0, int x1, int y1, TGAImage &image, const TGAColor &color) {
	...
	if (std::abs(dx) >= std::abs(dy)) {
		...
		for (int i = 0; i <= std::abs(dx); ++i)
			image.set(x0+sign(dx)*i, y0+std::round(sign(dy)*i*diff), color);
	}
	else {
		float diff = std::abs(1.0f*dx/dy);
		for (int i = 0; i <= std::abs(dy); ++i)
			image.set(x0+std::round(sign(dx)*i*diff), y0+sign(dy)*i, color);
	}
}
...
void filledTriangle(Vec2i p1, Vec2i p2, Vec2i p3, TGAImage &image, const TGAColor &color) {
	...
	float currx  = p1.x,
	      curry1 = p1.y,
	      curry2 = p1.y,
	      dy12   = float(p2.y-p1.y)/x12,
	      dy23   = float(p3.y-p2.y)/x23,
	      dy13   = float(p3.y-p1.y)/x13;
	...
	line(p1.x, p1.y, p1.x, p1.y, image, TGAColor(255, 255, 0, 255));
}
